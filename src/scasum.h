...
#define fetchx(i)  cx[i]
    ...
    if (parms.incx == 1) {
        ...
        for (i = ctaStart + tid; i < parms.n; i += totalThreads) {
            ...
            sum += fabsf(cuCrealf(tx)) + fabsf(cuCimagf(tx));
        }
    } else {
...
#if (CUBLAS_SCASUM_THREAD_COUNT & (CUBLAS_SCASUM_THREAD_COUNT - 1))
#error code requires CUBLAS_SCASUM_THREAD_COUNT to be a power of 2
#endif

    for (i = CUBLAS_SCASUM_THREAD_COUNT >> 1; i > 0; i >>= 1) {
        __syncthreads(); 
        if (tid < i) {
