...
#define fetchx(i)  sx[i]
    ...
    sx = parms.sx;
#endif
    totalThreads = gridDim.x * CUBLAS_ISAMAX_THREAD_COUNT;
    ctaStart = CUBLAS_ISAMAX_THREAD_COUNT * blockIdx.x;
    ...
    partialSMax[tid] = smax;

#if (CUBLAS_ISAMAX_THREAD_COUNT & (CUBLAS_ISAMAX_THREAD_COUNT - 1))
#error code requires CUBLAS_ISAMAX_THREAD_COUNT to be a power of 2
#endif

    for (i = CUBLAS_ISAMAX_THREAD_COUNT >> 1; i > 0; i >>= 1) {
        __syncthreads(); 
        if (tid < i) {
            smax = partialSMax[tid];
            imax = partialIMax[tid];
            xabs = partialSMax[tid + i];
            jmax = partialIMax[tid + i];
            if ((xabs > smax) || ((xabs == smax) && (jmax < imax))) {
                imax = jmax;
                smax = xabs;
            }
            partialSMax[tid] = smax;
    ...
    if (tid == 0) {
        parms.resMax[blockIdx.x] = partialSMax[tid];
