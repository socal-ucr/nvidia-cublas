 ...
 */
    ...
    for (i = 0; i < parms.n; i += IINC) {
        ...
        ii = i + blockIdx.x * CUBLAS_SSPMV_THREAD_COUNT;            
         ...
         * in many cases we'll only executed the loop body once, i.e. we'll
...
#else
                    if (ii >= jj) {
                        idx = ii + (((twoN - jj - 1) * jj) >> 1);
                    } else {
                        idx = jj + (((twoN - ii - 1) * ii) >> 1);
                    }
#endif
                    sdot += parms.AP[idx] * XX[jj-j];
                    jj++;
                }
            }
        }
