...
#define fetchy(i)  cy[i]
    ...
    cy = parms.cy;
#endif
    totalThreads = gridDim.x * CUBLAS_CDOTC_THREAD_COUNT;
    ctaStart = CUBLAS_CDOTC_THREAD_COUNT * blockIdx.x;
    ...
    if ((parms.incx == parms.incy) && (parms.incx > 0)) {
        ...
        if (parms.incx == 1) {
            ...
            for (i = ctaStart + tid; i < n; i += totalThreads) {
                ...
                sum = cuCaddf (sum, cuCmulf (ty, cuConjf (tx)));
            }
        } else {
...
#if (CUBLAS_CDOTC_THREAD_COUNT & (CUBLAS_CDOTC_THREAD_COUNT - 1))
#error code requires CUBLAS_CDOTC_THREAD_COUNT to be a power of 2
#endif

    for (i = CUBLAS_CDOTC_THREAD_COUNT >> 1; i > 0; i >>= 1) {
        __syncthreads(); 
        if (tid < i) {
